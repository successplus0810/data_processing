    ...
    with xw.App(visible=False) as app:
        ...
        for sheet_name in sheet_df_mapping.keys():
            ...
            if sheet_name in current_sheets:
                for df_data in data :
                    print(df_data)
                    if(df_data['cell_export']!='U4'): # do not export ISQUERY in each sheet, ONLY sheet SUMMARIZER
...
def fill_summary_sheet(supp_desc,summary_index_list,path_export_final,vendor_num,dict_sheet_summarizer):
    with xw.App(visible=False) as app:
        ...
        summary_index = 1
        for i,query_note in dict_sheet_summarizer.items():
            ...
            wb_from.sheets['Supplier Summary'].range('N'+str(summary_index+7)).value = query_note
            summary_index += 1
...
def remove_template_sheet(sheet_name,path_export_final):
    print('Start delete template sheet')
    with xw.App(visible=False) as app:
        wb = app.books.open(path_export_final)             
    ...
    #     print(e)
    return print('Done delete template sheet')
...
def rebate_item(df_merge,cd_ref,isquery):
    try:
        ...
        if 'query' in isquery.lower():
            notes = f'PROMO - The item {itemid_name} was on promotion during this time. As per previous promotion, at the same RRP, the vendor agreed to support scan rate. However, according to our records, no funding has been charged. Please see sales data and email evidence for more information.'
...
def add_vendor_to_item_import():
    ...
    df_import = df_import[['ITEMID', 'UOM', 'BRANDID', 'CLASSIFY_STATE', 'SUBSTATE', 'STARTDATE','ENDDATE', 'RRP', 'DEAL', 'BUNDLE_QTY', 'BUNDLE_PRICE', 'EXCEL_PATH','EMAIL_PATH', 'CLAIM_NUMBER','QUERY']]
    excel_file.close()
...
def main():
    ...
    for vendor in list_vendor:
        ...
        for i in range(1,int(max_claim_number_vendor)+1):
            ...
            df_excel['QUERY'] = df_excel['QUERY'].astype(str)
            ######## 20240409 sua lai cai note , query
            isquery = df_excel['QUERY'][0]
            if isquery == 'nan':
               isquery = ''
            ...
            # notes:
            dict_data_notes = rebate_item(df_merge,cd_ref,isquery)
            # category id 
            ...
            dict_sheet_feedback = {'df': isquery,'cell_export':'U4'}
            #classify 20k
            sum_eli = df_merge['ELI_CLAIM'].sum()
            sum_claimed = cd_ref['CLM_VAL'].sum()
            list_data = list_data_sales + list_data_state + list_data_item + [dict_data_notes] + [dict_data_category_id] + [dict_sheet_feedback]
        ...
        for key in dict_classify:
            if key == 'LESSTHAN20K':
                ...
                for df_element in dict_classify['LESSTHAN20K']:
                    ...
                    dict_sheet_summarizer_less[str(i_1)] = df_element[0][-2]['df']
                    i_1+= 1
                fill_summary_sheet(supp_desc,summary_index_list = i_1,path_export_final =path_export_final_lessthan20k 
                                                              ,vendor_num = vendor_num, dict_sheet_summarizer = dict_sheet_summarizer_less)
                remove_template_sheet('template',path_export_final_lessthan20k)
                i_insert = 8
                for key,value in dict_loc_insert_less.items():
                    ...
                    try:
                        insert_attachments(file_path_excel,excel_name,file_path_email,email_name,i_insert,path_export_final_lessthan20k)
                    except:
                        pass
                    i_insert+=2
                # move_worksheet_to_vba_template(path_xlsb = path_export_final_lessthan20k_xlsb)
            else:
                dict_sheet_summarizer_more = {}
                i_2 = 1
                for df_element in dict_classify['MORETHAN20K']:
                    ...
                    dict_sheet_summarizer_more[str(i_2)] = df_element[0][-2]['df']
                    i_2+= 1
                fill_summary_sheet(supp_desc,summary_index_list = i_2,path_export_final =path_export_final_morethan20k 
                                                              ,vendor_num = vendor_num, dict_sheet_summarizer = dict_sheet_summarizer_more)
                remove_template_sheet('template',path_export_final_morethan20k)
                i_insert_2 = 8
    ...
    df_check_prof_detail(max_claim_number,df_import_ven_1,dict_item_import_checked)           
    print('ENDDDDD')
    return dict_sheet_summarizer_less


if __name__ == '__main__':
    cursor = set_up(config = config_coles)
    dict_sheet_summarizer_less = main()